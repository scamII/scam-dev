				...
				<svg class="w-full max-w-2xl h-auto opacity-50" viewBox="0 0 480 200" fill="none" xmlns="http://www.w3.org/2000/svg">
					<defs>
						<linearGradient id="g1" x1="0" y1="0" x2="1" y2="1">
							<stop offset="0%" stop-color="#ff5743" stop-opacity="0.12" />
							<stop offset="100%" stop-color="#ff5743" stop-opacity="0.03" />
						</linearGradient>
						...
						<linearGradient id="horseGrad" x1="0" y1="0" x2="1" y2="1">
							<stop offset="0%" stop-color="#ff5743" stop-opacity="0.08" />
							<stop offset="100%" stop-color="#ff5743" stop-opacity="0.02" />
						</linearGradient>
					</defs>
					<style>
						@keyframes blink1 {
			...
			<div class="status-bar mb-8">
				...
				<span>UPTIME: <?php
								...
								if (is_readable('/proc/uptime')) {
									...
									if ($uptime_str !== false) {
										$parts = explode(' ', trim($uptime_str));
										$uptime_seconds = (int) floatval($parts[0]);
									}
								}
								if ($uptime_seconds < 60) {
									echo esc_html($uptime_seconds . ' сек');
								} elseif ($uptime_seconds < 3600) {
									echo esc_html((int)($uptime_seconds / 60) . ' мин');
								} elseif ($uptime_seconds < 86400) {
									$h = (int)($uptime_seconds / 3600);
									$m = (int)(($uptime_seconds % 3600) / 60);
									echo esc_html($h . ' ч ' . $m . ' мин');
								} elseif ($uptime_seconds < 31536000) {
									$d = (int)($uptime_seconds / 86400);
									$h = (int)(($uptime_seconds % 86400) / 3600);
									echo esc_html($d . ' д ' . $h . ' ч');
								} else {
									$y = (int)($uptime_seconds / 31536000);
									$d = (int)(($uptime_seconds % 31536000) / 86400);
									echo esc_html($y . ' лет ' . $d . ' д');
								}
								?></span>
			</div>
